$Folder = $env:USERPROFILE + "\zscaler-cert-app-store"
...
function Update-Apps{
    foreach ($Key in $appsenv.Keys) {
        ...
        try{
            $tmp = $appsenv[$key]
            $env = Get-Childitem Env:$tmp
            Write-Log "Enviroment variable: $($appsenv[$key]) already set to: $($env.Value) overwriting"
        }catch{
...
# Run Program
Main$Folder = $env:USERPROFILE + "\zscaler-cert-app-store"
$LogFile = "zcertlogs.txt"
$Store = Join-Path -Path $Folder -ChildPath "zscalerCAbundle.pem"
$appsenv = @{ openssl = "SSL_CERT_FILE"; 
                curl = "CURL_CA_BUNDLE"; 
